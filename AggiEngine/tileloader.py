        ...
        for layer in tiled_map:
            ...
            if isinstance(layer, pytmx.TiledTileLayer):
                for x, y, image in layer.tiles():
                    ...
                    gameObject.textureID = image[0]
                    gameObject.name = layer.name

                    gameObject.setWidth(image[1] / gameObjectHandler.scale ** 2)
                    gameObject.setHeight(image[2] / gameObjectHandler.scale ** 2)
                    ...
                    if bodyType != 'none':
                        ...
                        if color:
                            ...
                            gameObjectHandler.add(gameObject, bodyDef, bodyFixtureDef, color)
                        else:
                            gameObjectHandler.add(gameObject, bodyDef, bodyFixtureDef)
                    else:
                        gameObject.position = (-x / gameObjectHandler.scale, -y / gameObjectHandler.scale)
                        if color:
                            gameObject.textureID = -1
                            vertices = hitboxes[verticesName]
                            gameObject.vertices = [
                                (v[0] / gameObjectHandler.scale, v[1] / gameObjectHandler.scale) for v in vertices]
                            gameObjectHandler.add(gameObject, color=color)
                        else:
                            gameObjectHandler.add(gameObject)
